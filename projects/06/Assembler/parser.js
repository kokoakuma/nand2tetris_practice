...
class Parser {
  ...
  symbol() {
    if (this.commandType() === 'A_COMMAND') {
      ...
      throw new Error('commandType should be A or L');
    }
  }

  dest() {
    if (this.commandType() !== 'C_COMMAND') {
      throw new Error('commandType should be C when call dest');
    } else if (this.currentCommand.indexOf(';') !== -1) {
      return null;
    }
  ...
  jump() {
    if (this.commandType() !== 'C_COMMAND') {
      throw new Error('commandType should be C when call jump');
    }
    return this.currentCommand.split(';')[1];
  }
}